...
function finalizarCobrancaEntregador(id, forma) {
  ...
  // Registra a venda na hora que o entregador recebe
  vendas.push({
    id: Date.now(),
...
function renderizarProdutos(lista) {
  ...
  produtosParaExibir.forEach(prod => {
    ...
    const imgData = prod.imagem ? prod.imagem : "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect fill='%23eee' width='100' height='100'/><text fill='%23999' x='50' y='50' font-family='sans-serif' font-size='14' text-anchor='middle' alignment-baseline='middle'>Sem Foto</text></svg>";
...
function confirmarDelivery() {
  ...
  // Se já pagou no caixa, registra a venda imediatamente!
...
// ==========================================
// FLUXO DE PAGAMENTO (CAIXA FÍSICO)
// ==========================================
...
// ==========================================
// PAINEL ADM - DASHBOARDS
// ==========================================
function renderizarDashboard() {
  let totalGeral = 0;
  let porUsuario = {};
  let porProduto = {};
  let porPagamento = { "Pix": 0, "Cartão": 0, "Dinheiro": 0 };
  ...
  const renderGrafico = (divId, dataObj, color, prefix = "", suffix = "") => {
    const div = document.getElementById(divId); div.innerHTML = "";
    let maxV = Math.max(...Object.values(dataObj), 1);
    for (let k in dataObj) {
      let val = dataObj[k];
      let perc = (val / maxV) * 100;
      div.innerHTML += `<div class="barra-container"><div class="barra" style="height:${perc}%; background:${color};">${prefix}${Math.round(val)}${suffix}</div><span title="${k}">${k}</span></div>`;
    }
  };
  
  renderGrafico("grafico-pagamentos", porPagamento, "#f39c12", "R$");
  renderGrafico("grafico-usuarios", porUsuario, "#3498db", "R$");
...
function cadastrarProduto() {
  ...
  if (!nome || isNaN(preco) || isNaN(qtd)) return alert("❌ Preencha Nome, Preço e Quantidade.");
...
// ==========================================
// CONFIGURAÇÕES GERAIS E PAYLOAD PIX
// ==========================================
